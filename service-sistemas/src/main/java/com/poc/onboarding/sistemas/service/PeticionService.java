package com.poc.onboarding.sistemas.service;
...
import com.poc.onboarding.sistemas.repository.PeticionSistemasRepository;
...
public class PeticionService {
    ...
    private final PeticionSistemasRepository repository;
    private final RabbitTemplate rabbitTemplate;

    public List<PeticionSistemas> obtenerPendientes() {
        return repository.findByEstadoOrderByFechaCreacionDesc(EstadoPeticion.PENDIENTE);
    }

    public List<PeticionSistemas> obtenerHistorial() {
        return repository.findByEstadoNotOrderByFechaProcesamiento(EstadoPeticion.PENDIENTE);
    }

    public Optional<PeticionSistemas> obtenerPorId(Long id) {
    ...
    public void aprobar(Long id, String procesadoPor) {
        repository.findById(id).ifPresent(peticion -> {
            ...
            log.info("Petición SISTEMAS {} aprobada. Username generado: {}",
    ...
    public void denegar(Long id, String motivo, String procesadoPor) {
        repository.findById(id).ifPresent(peticion -> {
            ...
            log.info("Petición SISTEMAS {} denegada. Motivo: {}", peticion.getPeticionId(), motivo);
        });
    }

    @Transactional
    public PeticionSistemas guardar(PeticionSistemas peticion) {
    ...
    public void eliminarPorWorkflowId(String workflowId) {
        repository.findByWorkflowId(workflowId).ifPresent(peticion -> {
            ...
            log.info("Petición SISTEMAS eliminada por compensación. WorkflowId: {}", workflowId);
        });
    }
}
